...
describe('draft-04: formats', function () {
  this.timeout(10000)
  ...
  it('should display the doclink to the draft-04-schema', function () {
    expect(bptest.$(select('format-date-time', 'format', '.header a')).attr('href'))
      .to.equal('https://tools.ietf.org/html/draft-fge-json-schema-validation-00#section-7')
  })
  ...
  function checkFormat (definitionId, expectedFormatName, expectedSection) {
    ...
    it('should display the doclink to the draft-04-schema', function () {
      expect(bptest.$(select(definitionId, 'format', '> .contents a')).attr('href'))
        .to.equal(`https://tools.ietf.org/html/draft-fge-json-schema-validation-00#section-${expectedSection}`)
    })
  }
})
